...
<main class="container mx-auto px-4">
        <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            while ($auteur = $requete->fetch()) {
                echo '<li class="bg-white rounded-lg shadow p-4">';
                echo '<img src="' . $auteur['lien_avatar'] . '" alt="Image de l\'auteur" class="h-200">';
                echo '<p class="text-lg font-semibold">' . $auteur['prenom'] . ' ' . $auteur['nom'] . '</p>';
                if (!empty($auteur['lien_twitter'])) {
                    echo '<a href="' . $auteur['lien_twitter'] . '" class="text-blue-500 hover:underline">Compte Twitter</a>';
                }
